...
public enum SportFields implements EntityFieldsInterface {

    /**
     * ID.
     * Identifier of a sport object.
     */
     ...
     */
    NAME("Sport name", "name", PermissionLevel.VIEW_ANONYMOUS, true, true, false, false, DataTypeExtended.STRING, null);
     ...
     */
    SportFields(String displayName, String databaseField, PermissionLevel minimumPermission, boolean creatable, boolean editable, boolean nullAllowed, boolean emptyAllowed, DataTypeExtended dataType, AbstractEntityManifest referenceManifest) {
    ...
    public static SportFields[] valuesAllowed() {
        // Create a list of allowed values
        List<SportFields> list = new ArrayList<>();
        ...
        for(SportFields value : values())
            if(value.getMinimumPermission().orBetter(permissionLevel))
                list.add(value);

        // Return the values array
        return list.toArray(new SportFields[]{});
    }
}
